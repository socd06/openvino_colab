...
def emotion(input_image):
    '''
    Given some input image, preprocess the image so that
    it can be used with the related emotion recognition model
    you downloaded previously. You can use cv2.resize()
    to resize the image.
    '''
    preprocessed_image = np.copy(input_image)

    # TODO: Preprocess the image for the emotion recognition model
    preprocessed_image = preprocessing(preprocessed_image, 64, 64)
    return preprocessed_image
